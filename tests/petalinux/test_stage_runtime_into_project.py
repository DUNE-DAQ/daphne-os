...
class StageRuntimeIntoProjectTests(unittest.TestCase):
    ...
    def test_previous_qualified_source_requires_matching_old_contract(self) -> None:
        # The prior ABI-compatible server still implicitly enables bias in
        # Configure. Matching minor capabilities cannot substitute for this pin.
        bundle, _ = self.make_bundle()
        metadata = bundle.parent / "BUILD-METADATA.txt"
        metadata.write_text(metadata.read_text().replace(
            REQUIRED_COMMIT, "4e74f1071d09e16a15db597204c22debead2592c"))
        result = self.run_stage(bundle)
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("does not match required commit", result.stderr)
        self.assert_prior_state_preserved()

    def test_previous_sc_ownership_runtime_cannot_replace_mezzanine_status_runtime(self) -> None:
        # ABI-minor compatibility alone does not supply the corrected producer.
        bundle, _ = self.make_bundle()
        metadata = bundle.parent / "BUILD-METADATA.txt"
        metadata.write_text(metadata.read_text().replace(
    ...
    def test_previous_abi22_runtime_without_management_link_requires_old_pin(self) -> None:
        bundle, _ = self.make_bundle()
        metadata = bundle.parent / "BUILD-METADATA.txt"
        metadata.write_text(metadata.read_text().replace(
            REQUIRED_COMMIT, "3f636f4acf3f795e96d1e20e89936c6a861ac58a"))
        result = self.run_stage(bundle)
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("does not match required commit", result.stderr)
        self.assert_prior_state_preserved()
