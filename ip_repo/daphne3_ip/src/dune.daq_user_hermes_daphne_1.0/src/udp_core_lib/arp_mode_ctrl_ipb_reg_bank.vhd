...
use work.ipbus_decode_arp_mode_ctrl_mmap_ipb.all;
use work.udp_core_ipb_rst_pkg.all;
...
begin
    ...
    arp_mode_ctrl_bank : ipbus_ctrlreg_v
    generic map(
    ...
    timeout_nz_bank : ipbus_ctrlreg_nz_bank
    generic map(
        N_REG => N_NZ_REG
    )
    port map(
    ...
    arp_mode_entry_bank : ipbus_ctrlreg_v
    generic map(
        N_CTRL => 0,
