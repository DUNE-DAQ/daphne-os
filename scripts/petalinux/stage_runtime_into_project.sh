...
if [[ -f "$SOURCE_METADATA" ]]; then
  ...
  # One unambiguous execution record. Preserve old QEMU records, but never
  # relabel native board execution as emulation. This checks a declaration;
  # staging neither executes the binary nor authenticates the evidence.
  if metadata_has_key execution_validation_kind || metadata_has_key execution_validation; then
    if metadata_has_key qemu_validation; then
      echo "ERROR: runtime metadata mixes legacy and explicit execution validation records" >&2
      exit 2
    fi
    execution_kind="$(metadata_value execution_validation_kind)"
    execution_validation="$(metadata_value execution_validation)"
    if [[ "$execution_kind" != native-aarch64 && "$execution_kind" != qemu-aarch64 ]]; then
      echo "ERROR: unsupported execution_validation_kind; expected native-aarch64 or qemu-aarch64" >&2
      exit 2
    fi
  else
    execution_kind="qemu-aarch64"
    execution_validation="$(metadata_value qemu_validation)"
  fi
  ...
  if [[ ! "$execution_validation" =~ ^PASS:[[:space:]]*[^[:space:]].*$ ]]; then
    echo "ERROR: runtime metadata must contain a passing execution validation record with a description" >&2
    exit 2
  fi
...
DAPHNE_SERVER_RUNTIME_EXECUTION_KIND = "$execution_kind"
...
Release qualification:
  $([[ "$qualified" == "1" ]] && printf '%s' "PASS (server $metadata_server_commit, gateware ABI $required_abi, recorded execution $execution_kind)" || printf '%s' "UNQUALIFIED (adjacent BUILD-METADATA.txt was not supplied)")
  Metadata checks only; staging performs no executable or hardware tests.
