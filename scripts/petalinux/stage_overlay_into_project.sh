...
RESOLVED_ABI_MINOR=0
RESOLVED_IDENTITY=""
IDENTITY_HELPER="$(dirname -- "${BASH_SOURCE[0]}")/gateware_identity.py"
...
resolve_bundle() {
  ...
  if [[ -e "$RESOLVED_DIR/GATEWARE-IDENTITY.json" ]]; then
    RESOLVED_IDENTITY="$RESOLVED_DIR/GATEWARE-IDENTITY.json"
    checksum_paths+=("${RESOLVED_APP}/GATEWARE-IDENTITY.json")
    if [[ -e "$RESOLVED_DIR/post_route_timestamp_snapshot.rpt" ]]; then
      checksum_paths+=("${RESOLVED_APP}/post_route_timestamp_snapshot.rpt")
    fi
  elif [[ -e "$RESOLVED_DIR/post_route_timestamp_snapshot.rpt" ]]; then
    echo "ERROR: snapshot report without sealed identity metadata" >&2
    exit 2
  fi
  ...
  if [[ -n "$RESOLVED_IDENTITY" ]]; then
    local variant=1
    [[ "$label" == self-trigger ]] || variant=2
    RESOLVED_ABI_MINOR="$(python3 "$IDENTITY_HELPER" verify \
      --record "$RESOLVED_IDENTITY" --binary "$RESOLVED_DIR/${RESOLVED_APP}.bin" \
      --report "$RESOLVED_DIR/post_route_timestamp_snapshot.rpt" \
      --build-sha "$RESOLVED_SHA" --variant "$variant")"
  fi
}
...
self_abi_minor="$RESOLVED_ABI_MINOR"
self_identity="$RESOLVED_IDENTITY"
...
full_abi_minor="$RESOLVED_ABI_MINOR"
full_identity="$RESOLVED_IDENTITY"
...
stage_one() {
  ...
  local variant="$8"
  local abi_minor="$9"
  local identity_file="${10}"
  local destination="$temporary_staged/$mode"
  local identity_origin=legacy-abi20-bundle
  local -a staged_files=("${app}.bin" "${app}.dtbo" shell.json BUILD-METADATA.txt)
...
stage_one self-trigger "$self_app" "$self_sha" "$self_firmware_name" "$self_dir" "$self_zip" "$self_manifest" 1 "$self_abi_minor" "$self_identity"
stage_one full-stream "$full_app" "$full_sha" "$full_firmware_name" "$full_dir" "$full_zip" "$full_manifest" 2 "$full_abi_minor" "$full_identity"
...
DAPHNE_SELF_TRIGGER_ABI_MINOR = "$self_abi_minor"
DAPHNE_FULL_STREAM_ABI_MINOR = "$full_abi_minor"
DAPHNE_SELF_TRIGGER_IDENTITY_SEALED = "$([[ -n "$self_identity" ]] && echo 1 || echo 0)"
DAPHNE_FULL_STREAM_IDENTITY_SEALED = "$([[ -n "$full_identity" ]] && echo 1 || echo 0)"
EOF
...
render_profile() {
  ...
  local abi_minor="$5"
  local app_count profile_count mode_count major_count minor_count
  ...
  awk -v app="$app" -v minor="$abi_minor" '{ if ($0 ~ /^APP=/) print "APP=" app;
       else if ($0 ~ /^IDENTITY_ABI_MINOR=/) print "IDENTITY_ABI_MINOR=" minor; else print }' \
    "$source" > "$destination"
  chmod 0644 "$destination"
}

render_profile "$SELF_PROFILE" "$temporary_self_profile" "$self_app" self-trigger "$self_abi_minor"
render_profile "$FULL_PROFILE" "$temporary_full_profile" "$full_app" full-stream "$full_abi_minor"
