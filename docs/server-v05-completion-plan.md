...
| --- | --- | --- |
| SC BiasEnable ownership | Aggregate DAQ Configure must not choose SC enable; retain I288 readback and distinguish command evidence from requested SC state | Candidate fb82e0a removes the implicit enable write and updates fingerprint/client verification. Clean 33 host/33 actual ARM suites and 216 Python tests per binding pass; installed 4e74f10 and SC state remain unchanged. Server-only maintenance/live preservation, packaging/pin/ONL and the dedicated/authenticated SC request contract remain pending. See sc-bias-enable-ownership.md |
...
| Qualification and handoff | Native/ARM tests, negative/stale/concurrency cases, live zero-BIAS/all-channel regression, both-ABI scope recorded, protocol clients/docs/wiki and ONL bundle | Server 4e74f10 native/live ABI 2.0 regression, runtime/native loader, image pin, 140 packaging/audit tests, 35-file ONL handoff and five exported clients pass. Prior 78504f1 handoff is unchanged. New-firmware/full-stream/image qualification and remaining hardware/database evidence are still pending |
