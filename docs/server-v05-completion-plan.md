...
| --- | --- | --- |
| Kernel/OS identification / I091–I093 | Per-field quality/times for running kernel and OS release; keep original base/image labels separate from current rootfs integrity | Candidate 75972de: seven typed observations, clean 32 host/32 actual ARM suites, 197 Python tests per binding and two native OS metadata reads pass. Not deployed; live/runtime qualification remains pending. I093 is partial; no approved installed-rootfs manifest and optional base/image labels absent on DAPHNE-015. See host-software-verification.md |
...
| Qualification and handoff | Native/ARM tests, negative/stale/concurrency cases, live zero-BIAS/all-channel regression, both-ABI scope recorded, protocol clients/docs/wiki and ONL bundle | Server 702155b native/live ABI 2.0 regression, complete runtime/native loader, image pin and 26-file ONL source/client/runtime handoff pass; wiki published. Prior eecff61 handoff remains unchanged. New-firmware/full-stream/image qualification and remaining hardware/database evidence are still pending |
