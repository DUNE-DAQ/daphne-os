...
| Management Ethernet / workbook I071–I084 | Read-only state/carrier/speed/duplex/MTU, traffic/error/drop and carrier-change counters; conservative health semantics | Source bffea24: 27 host/27 native ARM C++ suites and 144 Python tests pass; two native board samples validate all 14 fields. New server cross-build passes but service/RPC deployment remains pending. No network changes. See management-link-verification.md |
...
| Qualification and handoff | Native/ARM tests, negative/stale/concurrency cases, live zero-BIAS/all-channel regression, both-ABI scope recorded, protocol clients/docs/wiki and ONL bundle | Server 3f636f4 native/live ABI 2.0 regression, matching client bindings, private-safe ONL handoff and wiki are complete. New-firmware/full-stream/image qualification and remaining hardware/database evidence are still pending |
