...
| FPGA health | Actual loaded/admitted FPGA identity, configuration state, clocks/reset/timing, transport/link observations and clear degraded/unknown reasons; no service-active shortcut | Deployed 78504f1 retains 15 checks. Candidate 4e74f10 adds fresh admitted AFE-reset assessment: 33 host/33 actual ARM suites, 208 Python tests per binding and two independent-audited native probes pass. Candidate deployment/full RPC regression and handoff pending; see afe-reset-health-verification.md. Coherent live timestamp, Hermes delivery and external-reset epoch remain unknown; no overall health/run-permit claim |
...
| Qualification and handoff | Native/ARM tests, negative/stale/concurrency cases, live zero-BIAS/all-channel regression, both-ABI scope recorded, protocol clients/docs/wiki and ONL bundle | Server 78504f1 native/live ABI 2.0 regression, runtime/native loader, image pin, 32-file ONL handoff/four exported clients and wiki pass. Native-qualified AFE-reset candidate 4e74f10 is not deployed or packaged; its full RPC/handoff qualification remains pending. Prior handoffs are unchanged. New-firmware/full-stream/image qualification and remaining hardware/database evidence are still pending |
