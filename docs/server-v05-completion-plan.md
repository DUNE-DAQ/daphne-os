...
| Database identity | Authoritative crate/slot/detector, management IP, timing address and per-Hermes MAC/IP; typed provenance/revision and unavailable handling; distinguish assigned from observed, never change network identity implicitly | Explicit-field review extractor implemented/tested on ONL; placement and board/sender DNS agree. Server protocol integration, timing/management-MAC assignment and physical-link mapping remain pending |
