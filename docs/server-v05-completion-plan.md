...
| --- | --- | --- |
| SC BiasEnable ownership | Aggregate DAQ Configure must not choose SC enable; retain I288 readback and distinguish command evidence from requested SC state | Deployed fb82e0a removes the implicit enable write and updates fingerprint/client verification. Clean 33 host/33 actual ARM suites, 216 Python tests per binding and full live preservation/regression pass. Server-only deployment preserved firmware/runtime/Hermes and enable1. Exact runtime/native loader, image pin 1039f46, 140 packaging tests and 41-file ONL handoff/five exported clients pass. Dedicated/authenticated SC request handling remains pending. See sc-bias-enable-ownership.md |
| Complete Server Platform row assessment | Preserve every original meaning and distinguish producer coverage from hardware/mode qualification | All 251 rows recorded: 100 implemented, 84 partial, 54 missing, 13 contract-pending. Candidate 79f6e5d connects coherent mezzanine monitoring/status/client; 34 host/34 actual ARM suites and 240 Python tests per binding pass. No deployment or live candidate RPC qualification; deployed server remains fb82e0a. Exact XLSX/export comparison and audit guards preserve original meanings; see server-v05-row-audit.md. This does not close the remaining requirements |
| Mezzanine monitoring / I203–I214 | One quality/timed per-block sample with invalidation and independently retained alert history; preserve existing protective actions | Candidate 79f6e5d integrates driver 6514092 through server 97831dc and validated CLI/GUI logic. 34 host/34 actual ARM suites, 240 Python tests per binding and unchanged board guards pass. I207–I214 source producers implemented; I203/I204 remain partial physical-state/protection observations. Live candidate RPC/regression, deployment/runtime/ONL and actual Qt event-loop/fitted-hardware qualification remain pending. See mezzanine-status-verification.md |
| Mezzanine calibration / I225–I226 | Actual identity-bracketed INA232 calibration register pair with quality/time; separate requested values; no alert clearing or configuration writes | Candidate de420e0: 34 host/34 actual ARM suites and 222 Python tests per binding pass. Six new fake-bus driver cases plus protocol/client checks cover errors, mismatch, time and concurrency. Not deployed; no mezzanines fitted. Measurement-cache coherence and protection validity remain open; see mezzanine-calibration-readback.md |
