...
BIASCTRL, and the existing SC-requested generator state. Commit in small steps.

| Requirement | Implementation / evidence needed | State |
| --- | --- | --- |
| SC BiasEnable ownership | Aggregate DAQ Configure must not choose SC enable; retain I288 readback and distinguish command evidence from requested SC state | Original fb82e0a removes the implicit enable write and updates fingerprint/client verification; deployed 79f6e5d retains it and repeats live preservation successfully. The 79f6e5d runtime/native loader, pin cb8958c and 46-file ONL handoff/six clients are now verified. The following historical counts remain fb82e0a. Clean 33 host/33 actual ARM suites, 216 Python tests per binding and full live preservation/regression pass. Server-only deployment preserved firmware/runtime/Hermes and enable1. Exact runtime/native loader, image pin 1039f46, 140 packaging tests and 41-file ONL handoff/five exported clients pass. Dedicated/authenticated SC request handling remains pending. See sc-bias-enable-ownership.md |
| Complete Server Platform row assessment | Preserve every original meaning and distinguish producer coverage from hardware/mode qualification | All 251 rows recorded: 100 implemented, 84 partial, 54 missing, 13 contract-pending. Candidate 79f6e5d connects coherent mezzanine monitoring/status/client; 34 host/34 actual ARM suites and 240 Python tests per binding pass. Deployed 79f6e5d passes live no-mezzanine RPC/CLI and full SC-preserving regression; its exact runtime/native loader, pin cb8958c, 142 packaging tests and 46-file ONL handoff/six clients are verified. Exact XLSX/export comparison and audit guards preserve original meanings; see server-v05-row-audit.md. This does not close the remaining requirements |
| Mezzanine monitoring / I203–I214 | One quality/timed per-block sample with invalidation and independently retained alert history; preserve existing protective actions | Candidate 79f6e5d integrates driver 6514092 through server 97831dc and validated CLI/GUI logic. 34 host/34 actual ARM suites, 240 Python tests per binding and unchanged board guards pass. I207–I214 source producers implemented; I203/I204 remain partial physical-state/protection observations. 79f6e5d is now deployed; live no-mezzanine RPC/CLI and full SC-preserving regression pass. Exact runtime/pin/ONL handoff and six exported clients are verified; actual Qt event-loop/fitted-hardware qualification remains pending. See mezzanine-status-verification.md |
| Mezzanine calibration / I225–I226 | Actual identity-bracketed INA232 calibration register pair with quality/time; separate requested values; no alert clearing or configuration writes | Candidate de420e0: 34 host/34 actual ARM suites and 222 Python tests per binding pass. Six new fake-bus driver cases plus protocol/client checks cover errors, mismatch, time and concurrency. Historical de420e0 evidence; correction included in deployed 79f6e5d, with coherent monitoring and live no-mezzanine checks. No fitted hardware; physical protection/metrology remain open; the new runtime/pin/ONL handoff is verified; see mezzanine-calibration-readback.md |
| AFE global readback / I283–I288 | Fresh quality/timed POWERSTATE, reset, SPI busy and BiasEnable register observations; SC003 owns requested energization | Deployed 78504f1: clean 33 host/33 actual ARM suites, 203 Python tests per binding, native probes and live AFE RPC/full zero-BIAS regression pass. Complete runtime/native loader, image pin fb53223 and 140 packaging/audit tests pass. ONL handoff and four exported clients verified; physical transitions/full-stream/new-firmware qualification remain pending. See afe-global-readback-verification.md |
| Kernel/OS identification / I091–I093 | Per-field quality/times for running kernel and OS release; keep original base/image labels separate from current rootfs integrity | Deployed 75972de: seven typed observations, clean 32 host/32 actual ARM suites, 197 Python tests per binding, native/live metadata and full zero-BIAS/all-channel regression pass. Complete runtime/native loader, image pin and 128 packaging tests pass; 30-file ONL handoff/wiki and exported OS/clock/build client checks pass. I093 remains partial: no approved installed-rootfs manifest and optional base/image labels absent. See host-software-verification.md |
...
| Host clock / workbook I086/I087/I090 and I098–I100 | Local clock/derived boot observations, kernel synchronization evidence, actual timesync-service source/sample/offset reporting with explicit freshness and privacy | Deployed 702155b: 31 host/31 actual ARM suites, 185 Python tests per binding, live clock/privacy RPC and full zero-BIAS/all-channel/six-suite regression pass. Complete runtime native loader, image pin, 127 packaging tests and 26-file ONL handoff/wiki pass; exported clock client also passes from ONL. Board reports May 2025, unsynchronized and no processed NTP samples; offset/age unavailable. No clock/NTP changes. Approved-source/fresh NTP evidence remains pending; see timesync-verification.md |
...
| FPGA health | Actual loaded/admitted FPGA identity, configuration state, clocks/reset/timing, transport/link observations and clear degraded/unknown reasons; no service-active shortcut | Deployed 4e74f10 has 16 checks including fresh admitted AFE reset: 33 host/33 actual ARM suites, 208 Python tests per binding, native probes and full live RPC/zero-BIAS regression pass. Runtime/native loader, image pin ecff25e, 140 packaging/audit tests, 35-file ONL handoff and five exported clients pass. Health is 12 PASS/1 external-timing FAIL/3 UNKNOWN. See afe-reset-health-verification.md; no overall health/run-permit claim |
...
| Qualification and handoff | Native/ARM tests, negative/stale/concurrency cases, live zero-BIAS/all-channel regression, both-ABI scope recorded, protocol clients/docs/wiki and ONL bundle | Server fb82e0a native/live ABI 2.0 SC preservation and full regression pass; exact runtime/native loader, image pin 1039f46, 140 packaging tests and 41-file ONL handoff/five exported clients pass. Eighteen ONL read-only exchanges and before/after guards preserve process/boot/valid FE hash. The prior 4e74f10 runtime/pin/35-file handoff remains unchanged and lacks this fix. New-firmware/full-stream/image qualification and remaining hardware/database evidence are still pending |
