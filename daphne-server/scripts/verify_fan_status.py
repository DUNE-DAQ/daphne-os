#!/usr/bin/env python3
"""Three read-only RPCs verify fresh fan-register reporting, not physical fan health.

No bias/PWM writes, private-detail opt-ins, I2C scans or service changes.
"""
import argparse
import json
...
from fan_status import check_fans, require
...
def main():
    ...
    try:
        ...
        for _ in range(2):
            ...
            reports.append(check_fans(status, high, state.observed_monotonic_ns))
            require(all(f.acquisition_started_monotonic_ns > prior.observed_monotonic_ns for f in status.fans))
            prior = state
        print(json.dumps({"register_reporting_verified": True, "physical_fan_health_verified": False,
                          "exchanges": sequence, "same_process_boot_and_configuration_evidence": True,
                          "observations": reports}, indent=2))
    finally:
        socket.close()
        context.term()
...
if __name__ == "__main__":
    try:
        ...
        print("Fan register verification failed; response and private details suppressed", file=sys.stderr)
        raise SystemExit(1)
